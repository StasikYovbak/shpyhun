...
| ![бос](shot-boss.png) | ![метро](shot-metro.png) |

* Роздільність рендеру **480×270**, цілочисельний масштаб у фізичних пікселях + letterbox, `imageSmoothingEnabled = false`.
* Тільки горизонтальна орієнтація, керування — віртуальний джойстик + три кнопки, повний мультитач.
...
|---|---|---|
| Рух | джойстик у лівій половині (зона появи — будь-де, мертва зона 12 px, радіус 60 px) | `A`/`D`, `←`/`→` |
| Присісти / зістрибнути з платформи | різкий рух джойстика вниз | `S`, `↓` |
| Стрибок | кнопка **A** | `W`, `↑`, `Пробіл`, `K` |
| Клинок «Арк-тесак» | кнопка **B** | `J` |
| Рейкострил | кнопка **C** (утримання — заряд) | `L` |
| Ривок | свайп по правій половині або подвійний тап по джойстику | `Shift` |
| Пауза | кнопка в кутку | `Esc`, `P` |

Розмір кнопок (S/M/L), розкладка для лівші, гучність звуку й музики та вібрація —
у налаштуваннях; усе зберігається в `localStorage` (у `try/catch`).
...
| # | Секція | Що робить |
|---|---|---|
| 0 | Константи | `VW/VH/TS`, фізика `PH`, клинок `BL`, рейкострил `RG`, утиліти, детермінований ГПВЧ |
...
| 8 | `Input` | мультитач (джойстик + 3 кнопки + свайп-ривок) і дублююча клавіатура, фронти натискань |
| 9 | Пули | кулі, вороги, аптечки, промені, телеграфи — масиви з компактуванням |
| 10 | Герой | стан, стрибок з coyote/буфером, ривок, обидві зброї, парирування, розряд, активне перезаряджання |
...
4. **Прохідність усіх 10 рівнів.** Математика: висота стрибка `JUMP²/(2·GRAV) =
   380²/3000 = 48,1 px = 3 тайли`, час підйому 0,253 с, падіння з 48 px — 0,218 с,
   довжина стрибка на максимальній швидкості `130 × 0,471 = 61 px ≈ 3,8 тайла`.
   Карти будуються з обмеженнями: прірва ≤ 3 тайли (48 px), уступ угору ≤ 2 тайли
   (32 px) — це перевіряє генератор. Понад те, `tools/reach.mjs` будує граф
   поверхонь **справжньою фізикою гри** (реальні `moveX`/`moveY`), симулюючи ходьбу,
   стрибки трьох висот і ривок, і пошуком у ширину доводить, що з точки появи
   досяжні і чекпоінт, і вихід — на всіх 10 рівнях. ✔
